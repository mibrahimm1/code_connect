...
export function setupSignaling(io: Server) {
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('validate-room', (payload: { roomId: string; password?: string; mode: 'create' | 'join' }, callback: (response: { success: boolean; message?: string }) => void) => {
            const { roomId, password, mode } = payload;
        ...
        socket.on('join-room', (payload: { roomId: string; password?: string }) => {
            ...
            // If room doesn't exist, create it (assuming validation passed or it's a new create)
            if (!room) {
                room = { id: roomId, password, participants: [] };
                rooms[roomId] = room;
            }
